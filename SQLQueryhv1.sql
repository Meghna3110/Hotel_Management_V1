...
DROP COLUMN IsPaymentCompleted;

ALTER TABLE Bookings
ADD IsPaymentCompleted INT;


SELECT * FROM Bookings;

ALTER TABLE Bookings
ADD CONSTRAINT DF_IsPaymentCompleted DEFAULT 1 FOR IsPaymentCompleted;

UPDATE Bookings
SET IsPaymentCompleted = 
    CASE 
        WHEN BookingId IN (SELECT BookingId FROM Payments) THEN 2  -- Payment Completed
        ELSE 1  -- Payment Pending
    END;
...
SET BookingStatus = 
    CASE 
        WHEN IsPaymentCompleted = 2 THEN 'Confirmed'
        WHEN IsPaymentCompleted = 1 THEN 'Pending'
        WHEN IsPaymentCompleted = 3 THEN 'Cancelled'
        ELSE 'Pending'  -- Default case
    END;



SELECT BookingId, IsPaymentCompleted, BookingStatus FROM Bookings;
...
ALTER COLUMN AmountPaid DECIMAL(10,2) NULL;
...
SELECT * FROM Rooms WHERE AvailabilityStatus = 1;
...
select *
 ...
 --ALL BOOKINGS WITH CUSTOMER DETAILS
 SELECT B.BookingId, C.FullName, R.RoomNumber, B.CheckInDate, B.CheckOutDate, B.BookingStatus
FROM Bookings B
...
UPDATE Bookings 
SET BookingStatus = 'Cancelled', IsPaymentCompleted = 3 
WHERE BookingId = 3;
